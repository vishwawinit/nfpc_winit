...
SUMMARY_KEYS = {'date_from', 'date_to', 'sales_org', 'route', 'user_code'}
...
def get_log_report(
    ...
    # --- Sales Summary from rpt_route_sales_summary_by_item (matches dashboard) ---
    f_summary = {k: v for k, v in filters.items() if k in SUMMARY_KEYS}
    sw, sp = build_where(f_summary, date_col='date')
    sales_row = query_one(
    ...
    # Returns/credit notes from RSIC (only table with GR/damage/expiry breakdown)
    ...
    returns_row = query_one(
        f"SELECT COALESCE(SUM(total_gr_sales + total_damage_sales + total_expiry_sales), 0) AS total_returns "
    ...
    # Current month sales (summary table, matches dashboard)
    ...
    mtd_f = {k: v for k, v in f_summary.items() if k != 'date_from' and k != 'date_to'}
    ...
    mtd_row = query_one(
        ...
        f"FROM (SELECT DISTINCT ON (route_code, item_code, date) total_sales "
        f"  FROM rpt_route_sales_summary_by_item WHERE {mw} "
        f"  ORDER BY route_code, item_code, date) AS d",
        mp
    )
