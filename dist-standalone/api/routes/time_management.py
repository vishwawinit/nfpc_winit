...
def get_time_management(
    ...
    rows = query(
        ...
        f"  CASE WHEN cv.first_checkin IS NOT NULL AND cv.last_checkout IS NOT NULL "
        f"    AND cv.last_checkout > cv.first_checkin "
        f"    THEN ROUND(EXTRACT(EPOCH FROM (cv.last_checkout - cv.first_checkin)) / 3600.0, 2) "
        ...
        f"    COALESCE("
        f"      MAX(CASE WHEN EXTRACT(YEAR FROM out_time) != 1900 THEN out_time ELSE NULL END), "
        f"      MAX(date + out_time::time) "
        f"    ) AS last_checkout, "
